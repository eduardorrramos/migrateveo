...
const hasVotedOnSighting = (sightingId: string): boolean => {
  const votedSightings = JSON.parse(localStorage.getItem('votedSightings') || '{}');
...
const confirmSighting = async (sighting: Sighting) => {
  ...
  const confirmed = window.confirm('Make sure this information is truthful. I confirm that this is accurate information');
  if (!confirmed) return;
...
const getStatusDisplay = (sighting: Sighting) => {
  if (!sighting.isActive) return { text: 'Inactive', classes: 'bg-red-100 text-red-800' };
  ...
  <div class="space-y-4">
    <div v-for="sighting in sightings" :key="sighting.id" 
      ...
      <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-6">
        <div class="flex-grow">
          <h3 class="text-xl font-semibold text-gray-900 mb-3">{{ sighting.location }}</h3>
          <div class="space-y-2">
